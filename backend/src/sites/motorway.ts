...
export function motorwayConfig(stagehand: any): SiteConfig {
  return {
    ...
    buildSearchUrl: (params: SearchParams) => {
      ...
      // STEP 1: Handle make parameter (optional)
      if (params.make) {
        ...
        // STEP 2: Handle model parameter (optional, but requires make)
        if (params.model) {
          ...
          const motorwayModelName = getMotorwayModelName(
            params.make,
            params.model
          );
          console.log(
        ...
        console.log("ℹ️ [Motorway] No make specified - will search all makes");
      }
     ...
     * Extracts car data from both Motorway auction and buy it now listings
     * Navigates to both list types and combines results
     * Uses the current page URL (which should have search parameters) and modifies listType
    ...
    extractCars: async (page: any) => {
      stagehand.log({
        category: "debug",
        message:
          "Starting extraction for Motorway from both auction and buy it now listings",
      });

      // Log the initial page URL
      const initialUrl = page.url();
      stagehand.log({
        category: "debug",
        message: `Initial page URL: ${initialUrl}`,
      });

      // Log that we're extracting from both listing types
      stagehand.log({
        category: "debug",
        message:
          "Will extract from both auction and buy it now listings using current search parameters",
      });

      const allCarData = [];

      // Helper function to extract cars from a specific page
      const extractCarsFromPage = async (page: any, listType: string) => {
        // Wait for page to stabilize before extraction
        await page.waitForTimeout(2000);

        // Check if we're on a valid page
        try {
          await page.waitForSelector('a[id^="vehicle_card_"]', {
            timeout: 10000,
          });
        } catch (error) {
          stagehand.log({
            category: "warn",
            message: `No vehicle cards found on ${listType} page - might be no results or wrong page`,
          });
          return [];
        }
        ...
        stagehand.log({
          category: "debug",
          message: `Found ${cards.length} vehicle cards on ${listType} page`,
        });
        const carData = [];
        ...
        for (const card of cards) {
          try {
            ...
            // Convert mileage string to number (remove "mi" and commas)
            const mileageNumber = mileage
              ? parseFloat(mileage.replace(/[mi,]/g, "")) || 0
              : 0;
            ...
            carData.push({
              ...
              imageUrl,
              title,
              price: parseFloat(price.replace(/[£,]/g, "")) || 0, // Convert price string to number
              location,
              registration: reg,
              mileage: mileageNumber, // Use converted mileage number
              source: "Motorway",
              listType: listType, // Add the list type to distinguish auction vs buy it now
      ...
      // Helper function to modify current page URL to use specific listing type
      const getListingUrl = (listType: string) => {
        // Get the current page URL which should already have all the search parameters
        const currentUrl = page.url();

        stagehand.log({
          category: "debug",
          message: `Current page URL: ${currentUrl}`,
        });

        // If we're not on a Motorway vehicles page, fall back to basic URL
        if (!currentUrl.includes("pro.motorway.co.uk/vehicles")) {
          stagehand.log({
            category: "warn",
            message: `Not on Motorway vehicles page, using fallback URL for ${listType}`,
          });
          return `https://pro.motorway.co.uk/vehicles?listType=${listType}`;
        }

        // Parse the current URL and modify the listType parameter
        ...
        stagehand.log({
          category: "debug",
          message: `Modified URL for ${listType}: ${finalUrl}`,
        });
      ...
      try {
        stagehand.log({
          category: "debug",
          message: "Extracting from auction listings with search parameters",
        });

        const auctionUrl = getListingUrl("auction");
        stagehand.log({
          category: "debug",
          message: `Navigating to auction URL: ${auctionUrl}`,
        });
        ...
        const auctionCars = await extractCarsFromPage(page, "auction");
        allCarData.push(...auctionCars);

        stagehand.log({
          category: "debug",
          message: `Extracted ${auctionCars.length} cars from auction listings`,
        });
      } catch (error) {
        stagehand.log({
          category: "warn",
          message: `Failed to extract auction listings: ${error}`,
        });
      }
