...
export function bcaConfig(stagehand: any): SiteConfig {
  return {
    ...
    useProxies: true, // BCA requires proxies
    ...
    buildSearchUrl: (params: SearchParams) => {
      ...
      const bqParts = ["VehicleType:Cars"];

      // Only add make filter if provided
      if (params.make && params.make.trim()) {
        const mappedMake = makeMapping[params.make] || params.make;
        bqParts.push(`Make:${mappedMake.toUpperCase()}`);

        // Only add model filter if both make and model are provided
        if (params.model && params.model.trim()) {
          const modelGroup =
            modelGroupMap[mappedMake.toUpperCase()]?.[params.model] ||
            params.model;
          bqParts.push(`ModelGroup:${modelGroup}`);
        }
      }
      ...
      (() => {
        if (params.minPrice && params.maxPrice) {
          bqParts.push(`CapCleanPrice:${params.minPrice}..${params.maxPrice}`);
        }
        if (params.minPrice) {
          bqParts.push(`CapCleanPrice:${params.minPrice}..9000000`);
        }
        if (params.maxPrice) {
          bqParts.push(`CapCleanPrice:0..${params.maxPrice}`);
        }
      })();
      ...
      (() => {
        if (params.minMileage && params.maxMileage) {
          bqParts.push(`Mileage:${params.minMileage}..${params.maxMileage}`);
        }
        if (params.minMileage) {
          bqParts.push(`Mileage:${params.minMileage}..9000000`);
        }
        if (params.maxMileage) {
          bqParts.push(`Mileage:0..${params.maxMileage}`);
        }
      })();
      ...
      (() => {
        ...
        if (params.minAge !== undefined && params.maxAge !== undefined) {
          ...
          bqParts.push(`DateRegistered:${minLabel}..${maxLabel}`);
        }
        if (params.minAge !== undefined) {
          const minLabel = closestLowerBand(params.minAge);
          bqParts.push(`DateRegistered:${minLabel}..99YEAR`);
        }
        if (params.maxAge !== undefined) {
          const maxLabel = closestUpperBand(params.maxAge);
          bqParts.push(`DateRegistered:0MONTH..${maxLabel}`);
        }
      })();

      // VAT qualification filter
      if (params.vatQualifying) {
        bqParts.push("VATType:VAT Qualifying");
      }
